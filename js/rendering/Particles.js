...
export class Particles{
 ...
 update(dt){for(let i=this.items.length-1;i>=0;i--){const p=this.items[i];p.userData.life-=dt;p.position.addScaledVector(p.userData.v,dt);p.userData.v.y-=8*dt;if(p.geometry.type==='SphereGeometry')p.userData.v.y+=9*dt;p.material.opacity=Math.max(0,p.userData.life*(p.geometry.type==='SphereGeometry'?.2:1.5));if(p.userData.life<=0){this.scene.remove(p);if(p.geometry!==this.cubeGeo)p.geometry.dispose();p.material.dispose();this.items.splice(i,1)}}}
}
