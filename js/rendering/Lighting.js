...
import {INFO} from "../world/Block.js?v=19";
export class Lighting{
 constructor(scene,world,cfg){
  ...
  this.fog=new THREE.FogExp2(0x87c9ef,.006);this.dynamic=new Map();this.lastScan="";this.maxLights=cfg.QUALITY?.maxLights||34;this.clock=0;this.glows=new Map();this.handLight=new THREE.PointLight(0xffb34d,0,10,2);this.handLight.position.set(.25,1.5,.35);scene.add(this.handLight);
 ...
 update(time,player,dt=.016){
  ...
  const key=`${Math.floor(player.pos.x/8)},${Math.floor(player.pos.y/4)},${Math.floor(player.pos.z/8)}`;if(key!==this.lastScan){this.lastScan=key;this.scan(player)}
 ...
 scan(player){for(const l of this.dynamic.values())this.scene.remove(l);for(const s of this.glows.values())this.scene.remove(s);this.dynamic.clear();this.glows.clear();const r=20,px=Math.floor(player.pos.x),py=Math.floor(player.pos.y),pz=Math.floor(player.pos.z),found=[];
